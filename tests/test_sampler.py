...
class TestSplitDataset(unittest.TestCase):
    ...
    def test_split_sizes(self):
        frames = [np.zeros((2, 2, 3), dtype=np.uint8) for _ in range(100)]
        labels = [0] * 50 + [1] * 50
    ...
    def test_deterministic_with_seed(self):
        frames = [np.zeros((2, 2, 3), dtype=np.uint8) for _ in range(50)]
...
class TestConcertSplit(unittest.TestCase):
    ...
    def test_splits_by_concert(self):
        c1_frames = [np.zeros((2, 2, 3), dtype=np.uint8) for _ in range(10)]
        c2_frames = [np.ones((2, 2, 3), dtype=np.uint8) for _ in range(5)]
        c1_labels = [0] * 10
        c2_labels = [1] * 5
    ...
    def test_requires_two_concerts(self):
        with self.assertRaises(ValueError):
            concert_split([[]], [[]], val_concert_idx=0)
